...
- **Excel-style column filters in the event table** — every column header has a small filter button (a funnel while that column is filtered). It opens a list of the column's distinct values as check boxes with counts, a search box and *(Select all)*. Filters on different columns are ANDed with each other and with every other filter in the app (type, actors, time range, profiles). The value list narrows by the other columns' filters, like Excel. A sort stays applied when the filters change. Clicking the column label still sorts. Clear a single column from its popup, or use *Clear All Column Filters* in the table's context menu; loading or clearing data resets them. Columns with more than 2000 distinct values list the first 2000. Events appended later (follow-file mode) are tested against the filters, and all column filters reset when the data is cleared.
...
- **Sorting an event table whose filter matched nothing showed every event again.** Sorting now keeps an empty filter result empty.
...
- **Dashboard tab's event breakdown now uses the configured type filter field.** It previously hardcoded `findByKey("level")` and a fixed ERROR/WARN/INFO/DEBUG vocabulary, so any log using a different type/level field name (or different value vocabulary) showed all-zero stats despite valid data. It now counts and displays whatever distinct values actually occur in `config::GetConfig().typeFilterField` (the same field the rest of the app's filtering/analysis panels already use), sorted by count.
